...
> **Gemessen am 13. 9. 2026, gegen den echten Anbieter:** `gpt-live-transcribe` hat **gar keine** Satzende-Erkennung – die Sitzungsanfrage mit `turn_detection` wird abgelehnt: „400: Turn detection is not supported for this transcription model." Das Modell streamt Deltas und schließt einen Satz erst auf `input_audio_buffer.commit` des Clients. Für den Zuhör-Automaten (Sprechbeginn und fertiger Satz kommen vom Anbieter) ist es damit ungeeignet; die Startempfehlung `gpt-transcribe` aus Abschnitt 2 gilt – und wurde ab diesem Tag auch gebaut.
