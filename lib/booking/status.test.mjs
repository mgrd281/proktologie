...
test("gültige Antwort: Zustand übernommen, Aufruf ohne Cookies und ohne Cache", async () => {
  ...
  assert.deepEqual(r, { bookingLive: true, bookingPaused: true, banner: "Geschlossen.", chatEnabled: true, voice: false });
...
test("leeres Banner wird zu null", async () => {
  ...
  assert.deepEqual(r, { bookingLive: false, bookingPaused: false, banner: null, chatEnabled: true, voice: false });
});
...
test("Der Chat-Schalter wird übernommen; fehlt er, gilt „an“", async () => {
  const aus = await fetchCockpitStatus("https://cockpit.example", {
    fetchImpl: async () => jsonResponse({ bookingLive: true, bookingPaused: false, banner: null, chatEnabled: false }),
  });
  assert.equal(aus.chatEnabled, false);
  const alt = await fetchCockpitStatus("https://cockpit.example", {
    fetchImpl: async () => jsonResponse({ bookingLive: true, bookingPaused: false, banner: null }),
  });
  assert.equal(alt.chatEnabled, true, "ältere Cockpit-Antwort ohne Feld");
});

test("Das Mikrofon gilt als aus, solange das Cockpit es nicht ausdrücklich meldet", async () => {
  // Umgekehrte Vorsicht als beim Chat: Ein Knopf, hinter dem nichts
  // passiert, ist schlimmer als kein Knopf – und Zuhören kostet je Minute.
  const lies = (extra) =>
    fetchCockpitStatus("https://cockpit.example", {
      fetchImpl: async () => jsonResponse({ bookingLive: true, bookingPaused: false, chatEnabled: true, ...extra }),
    });

  assert.equal((await lies({})).voice, false, "fehlendes Feld heißt aus");
  assert.equal((await lies({ voice: "ja" })).voice, false, "nur ein echtes true zählt");
  assert.equal((await lies({ voice: true })).voice, true);
});
