...
test("Die gewählte Sprache geht mit; erkannte Sprache und Fremdsprache kommen als Flaggen zurück", async () => {
  let seen = null;
  const r = await sendChat("https://cockpit.example", { ...BODY, lang: "en" }, {
    fetchImpl: async (url, init) => {
      seen = JSON.parse(init.body);
      return response({ ...OK, lang: "en", flags: { llm: "none", langDetected: true, foreign: "tr" } });
    },
  });
  assert.equal(seen.lang, "en");
  assert.equal(r.ok, true);
  assert.equal(r.answer.flags.langDetected, true);
  assert.equal(r.answer.flags.foreign, "tr");

  const ohne = await sendChat("https://cockpit.example", BODY, { fetchImpl: async () => response({ ...OK, flags: { llm: "none", langDetected: "ja", foreign: 5 } }) });
  assert.equal(ohne.answer.flags.langDetected, undefined, "nur echtes true zählt");
  assert.equal(ohne.answer.flags.foreign, undefined, "nur eine Zeichenkette zählt");
});

test("Sprachkanal: der Kanal geht mit, eine stille Antwort ohne Text ist gültig – ohne Flagge nicht", async () => {
  const silent = { reply: "", lang: "de", state: { v: 1, stage: "idle" }, flags: { llm: "none", silent: true } };
  let seen = null;
  const r = await sendChat("https://cockpit.example", { ...BODY, channel: "voice" }, {
    fetchImpl: async (url, init) => {
      seen = init;
  ...
  assert.equal(r.ok, true);
  assert.equal(r.answer.reply, "");
  assert.equal(r.answer.flags.silent, true);
  assert.equal(JSON.parse(seen.body).channel, "voice");

  const empty = await sendChat("https://cockpit.example", BODY, { fetchImpl: async () => response({ ...silent, flags: { llm: "none" } }) });
  assert.deepEqual(empty, { ok: false, error: "invalid" }, "leer ohne Flagge bleibt Unsinn");
  const text = await sendChat("https://cockpit.example", BODY, { fetchImpl: async () => response(OK) });
  assert.equal(text.answer.flags.silent, undefined);
});
