...
test("Schaltflächen, Formular und Notfall überleben den Seitenwechsel", () => {
  const store = fakeStore();
  const s = append(newSession("de"), "assistant", "Welche Uhrzeit passt Ihnen?", 1);
  save(store, {
    ...s,
    quick: [{ id: "time:2026-09-09|07:00", label: "07:00 Uhr" }],
    form: { id: "contact", title: "Ihre Kontaktdaten", submit: "Weiter", fields: [] },
    emergency: true,
  });
  const back = load(store);
  assert.deepEqual(back.quick, [{ id: "time:2026-09-09|07:00", label: "07:00 Uhr" }]);
  assert.equal(back.form.id, "contact");
  assert.equal(back.emergency, true);
});

test("Erfundene Schaltflächen und Formulare werden verworfen", () => {
  const roh = JSON.stringify({
    v: 1,
    sessionId: randomId(),
    lang: "de",
    messages: [],
    quick: [{ id: "ok", label: "Gut" }, { id: 5 }, "kaputt"],
    form: { id: "fremdes-formular", title: "X", fields: [] },
    emergency: "ja",
  });
  const s = parseSession(roh);
  assert.deepEqual(s.quick, [{ id: "ok", label: "Gut" }]);
  assert.equal(s.form, null, "unbekanntes Formular wird nicht übernommen");
  assert.equal(s.emergency, false, "nur echtes true zählt");
});

test("Die ausdrückliche Sprachwahl überlebt den Seitenwechsel – und nur echtes true zählt", () => {
  const store = fakeStore();
  const s = append(newSession("en"), "assistant", "Hello", 1);
  assert.equal(s.langChosen, false);
  save(store, { ...s, langChosen: true });
  assert.equal(load(store).langChosen, true);
  const roh = JSON.stringify({ v: 1, sessionId: randomId(), lang: "de", messages: [], langChosen: "ja" });
  assert.equal(parseSession(roh).langChosen, false);
});
