...
test("Intro zeigt das ganze Team: alle sechs deutlich sichtbar", () => {
  const states = Array.from({ length: MEMBER_COUNT }, (_, i) =>
    cardState(i, 0, DESKTOP, VIEW),
  );
  for (const [i, s] of states.entries()) {
    assert.ok(s.opacity > 0.3, `Ebene ${i} im Intro zu blass (${s.opacity.toFixed(2)})`);
    assert.ok(s.z < 0, `Ebene ${i} im Intro nicht in der Tiefe (z=${s.z})`);
  }
});

test("Intro: die Ebenen stehen rechts der Textspalte", () => {
  // Die dunkle Textspalte nimmt das linke Drittel ein. Die erste Ebene darf
  // mit ihrer Kante unter dem weichen Verlauf liegen (so wie in der Vorlage),
  // ihre MITTE muss aber rechts der Textspalte bleiben.
  const textColumnEdge = -VIEW.w * 0.2;
  for (let i = 0; i < MEMBER_COUNT; i++) {
    const p = project(cardState(i, 0, DESKTOP, VIEW), DESKTOP);
    assert.ok(
      p.x > textColumnEdge,
      `Ebene ${i} ragt in die Textspalte (x=${p.x.toFixed(0)} < ${textColumnEdge.toFixed(0)})`,
    );
    assert.ok(p.x < VIEW.w * 0.5, `Ebene ${i} liegt außerhalb rechts (x=${p.x.toFixed(0)})`);
  }
});

test("Intro: die Reihe wird nach hinten kleiner und überdeckt sich nicht", () => {
  const states = Array.from({ length: MEMBER_COUNT }, (_, i) =>
    cardState(i, 0, DESKTOP, VIEW),
  );
  const scales = states.map((s) => DESKTOP.perspective / (DESKTOP.perspective - s.z));
  for (let i = 1; i < scales.length; i++) {
    assert.ok(
      scales[i] < scales[i - 1],
      `Ebene ${i} ist nicht kleiner als ihre Vorgängerin (${scales[i].toFixed(3)} ≥ ${scales[i - 1].toFixed(3)})`,
    );
  }
  const xs = states.map((s) => project(s, DESKTOP).x);
  for (let i = 1; i < xs.length; i++) {
    assert.ok(xs[i] > xs[i - 1], `Ebene ${i} steht nicht rechts von ihrer Vorgängerin`);
  }
});

test("Lichtbahn setzt an den Unterkanten an, nicht in den Gesichtern", () => {
  for (let i = 0; i < MEMBER_COUNT; i++) {
    const s = cardState(i, 0, DESKTOP, VIEW);
    const foot = cardFootPoint(s, DESKTOP);
    assert.equal(foot.x, s.x);
    assert.equal(foot.z, s.z);
    assert.ok(foot.y > s.y, `Fußpunkt der Ebene ${i} liegt nicht unterhalb der Mitte`);
    // Genau die halbe Ebenenhöhe tiefer (4:5 bei cardWidth)
    assert.ok(Math.abs(foot.y - s.y - (DESKTOP.cardWidth * 1.25) / 2) < 0.001);
  }
});
